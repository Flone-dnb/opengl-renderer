...
glm::vec3 MathHelpers::convertDirectionToRollPitchYaw(const glm::vec3& direction) {
    ...
    if (!glm::epsilonEqual(length, 1.0F, lengthDelta)) [[unlikely]] {
        throw std::runtime_error("the specified direction vector should have been normalized");
    }
#endif

    glm::vec3 worldRotation = glm::vec3(0.0F, 0.0F, 0.0F);

    worldRotation.z = glm::degrees(std::atan2(direction.x, direction.z));
    worldRotation.y = glm::degrees(-std::asin(direction.y));
...
void MathHelpers::convertCartesianCoordinatesToSpherical(
    ...
    theta = glm::degrees(glm::atan2(location.x, location.z));
    phi = glm::degrees(glm::atan2(glm::sqrt(location.z * location.z + location.x * location.x), location.y));
}
...
glm::mat4x4 MathHelpers::buildRotationMatrix(const glm::vec3& rotation) {
    return glm::rotate(glm::radians(rotation.y), glm::vec3(0.0F, 1.0F, 0.0F)) *
