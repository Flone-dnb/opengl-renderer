...
public:
    ...
    /**
     * Called before a new frame is rendered to process movement input.
     *
...
private:
    ...
    inline void recalculateBaseVectorsForOrbitalCamera() {
        ...
        cameraRightDirection =
            glm::normalize(glm::cross(cameraForwardDirection, Globals::WorldDirection::up));
        cameraUpDirection = glm::cross(cameraRightDirection, cameraForwardDirection);
    }
